			...
			if (BloonsRunner.gamePhase.equals("game")) {
				for (int i = 0; i < BloonsRunner.currentBloons.size(); i++) {
					if (BloonsRunner.currentBloons.get(i).getCoordinates() != null && BloonsRunner.currentBloons.get(i).getCoordinates() != new int[]{-1, -1}) {
						int[] offset = BloonsRunner.currentBloons.get(i).calculateTileOffset(BloonsRunner.currentBloons.get(i).direction);

						int xCoordAdd = offset[0];
						int yCoordAdd = offset[1];

						g.setColor(BloonsRunner.currentBloons.get(i).getColor());
						g.fillOval((BloonsRunner.currentBloons.get(i).getCoordinates()[0] * BloonsRunner.PATH_WIDTH) + 15 + xCoordAdd, (BloonsRunner.currentBloons.get(i).getCoordinates()[1] * BloonsRunner.PATH_WIDTH) + 10 + yCoordAdd, BloonsRunner.PATH_WIDTH - 30, BloonsRunner.PATH_WIDTH - 20);
						g.fillOval((BloonsRunner.currentBloons.get(i).getCoordinates()[0] * BloonsRunner.PATH_WIDTH) + 22 + xCoordAdd, (BloonsRunner.currentBloons.get(i).getCoordinates()[1] * BloonsRunner.PATH_WIDTH) + 37 + yCoordAdd, 6, 6);
					}
				}
